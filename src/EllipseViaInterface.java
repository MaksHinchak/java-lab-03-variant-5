...
public final class EllipseViaInterface implements FunctionLike { // Ця реалізація не успадковує абстрактний Function.
    ...
    public EllipseViaInterface(double a, double b) { // Конструктор класу, який реалізує контракт.
    ...
    @Override public double value(double x) { // Обчислюємо верхню гілку так само, як у задачі 2.
        ...
        double result = b * Math.sqrt(Math.max(0, 1 - (x / a) * (x / a))); // Безпосередня формула без делегування абстрактному класу.
    ...
    @Override public String toString() { return "EllipseViaInterface(a=" + a + ", b=" + b + ")"; } // Описуємо тип і стан.
    @Override public boolean equals(Object other) { return other instanceof EllipseViaInterface f && Double.compare(a, f.a) == 0 && Double.compare(b, f.b) == 0; } // Порівнюємо за значенням.
